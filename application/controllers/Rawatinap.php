...
class Rawatinap extends CI_Controller
{
    ...
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Tbl_rawat_inap_model');
   ...
   public function index($level)
    {
        ...
        $config['total_rows'] = $this->Tbl_rawat_inap_model->total_rows($q);
        if ($poli == "POLI") {
            $pendaftaran = $this->Tbl_rawat_inap_model->get_limit_data_poli($config['per_page'], $start, $polipoli);
        }
        else{
            $pendaftaran = $this->Tbl_rawat_inap_model->get_limit_data($config['per_page'], $start, $q);
        }
    ...
    public function read() 
    {
        ...
        $row = $this->Tbl_rawat_inap_model->get_by_id($id);
        if ($row) {
            $data = array(
   ...
   function rujukan_action(){
    ...
    function cetakresepobat(){
     ...
     function cetakrujukan(){
      ...
      $sql_data_tindakan = "SELECT rt.id_poli, rt.kode_penyakit,rt.kode_tindakan,rt.no_rawat,rt.hasil_periksa,rt.nama_obat,rt.tanggal,p.nama_poli, pkt.nama_penyakit, t.nama_tindakan FROM tbl_riwayat_tindakan as rt, tbl_tindakan as t, tbl_poli as p, tbl_diagnosa_penyakit as pkt WHERE rt.no_rawat = '$no_rawat' AND rt.kode_tindakan = t.kode_tindakan AND rt.id_poli = p.id_poli AND rt.kode_penyakit = pkt.kode_diagnosa";
        ...
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(190, 7, 'PUSKESMAS MUNUJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(190, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(190, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
        $pdf->Line(20,35, 210-20, 35);
    ...
    public function update_action() 
    {
      ...
      $tanggal_daftar =  $this->input->post('tanggal_daftar',TRUE);
        ...
        'alamat_penanggung_jawab' => $this->input->post('alamat_penanggung_jawab',TRUE),
            ...
            $this->Tbl_rawat_inap_model->update($this->input->post('no_rawat', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tbl_rawat_inap_model->delete($id);
