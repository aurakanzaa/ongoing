...
class Tindakan extends CI_Controller
{
    ...
    public function filter_berdasarkan_poli()
    {
        $data['poli'] = $this->Tbl_poli_model->get_all();
        // $this->form_validation->set_rules('hidden', 'Hidden', 'trim|required');
        // if ($this->form_validation->run() == FALSE) {
            $this->template->load('template','tindakan/filter',$data);
        // } else {
        //     $poli = $this->input->post('id_poli');
        //     redirect('tindakan/filter_action/'.$poli,'refresh');
        // }
    }
    ...
    public function filter_action()
    {
        ...
        $this->form_validation->set_rules('hidden', 'Hidden', 'trim|required');
        if ($this->form_validation->run() == FALSE) {
            $this->template->load('template','tindakan/filter_action',$data);
        } else {
            $poli = $this->input->post('id_poli');
            $data['tindakan'] = $this->Tbl_tindakan_model->getByPoli($poli);
            $data['id_poli'] = $poli;
            $this->template->load('template','tindakan/filter_action',$data);
        }
    }
