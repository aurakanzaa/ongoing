...
class Pengadaanobat extends CI_Controller
{
    ...
    {
        $pengadaan = $this->Tbl_pengadaan_obat_model->getpengadaan();
        foreach($pengadaan as $peng){
            $id_pengadaan = $peng->id_pengadaan;
        }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() != FALSE) {
            ...
            $jumlah = $this->input->post('jumlah', TRUE);
            $satuan = $this->input->post('satuan', TRUE);
            $stokobat = $this->Tbl_pengadaan_obat_model->getStokObat($kode_obat);
            if (count($stokobat) > 0) {
                foreach ($stokobat as $key) {
                    $stok = $key->jumlah;
                }
                $stokbaru = $jumlah + $stok;
                $this->Tbl_pengadaan_obat_model->updateStokObat($kode_obat,$stokbaru);
            }
            else{
                $this->Tbl_pengadaan_obat_model->insertObat($kode_obat,$jumlah,$satuan);
            }
