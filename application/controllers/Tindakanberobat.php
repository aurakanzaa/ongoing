   ...
   public function index($level)
    {
        ...
        $poli = substr($level,0,4);
        $namapoli = substr($level,7);
        $polipoli = $poli." ".$namapoli;

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tbl_pendaftaran_rekmed_model->total_rows($q);
        if ($poli == "POLI") {
            $pendaftaran = $this->Tbl_pendaftaran_rekmed_model->get_limit_data_poli($config['per_page'], $start, $polipoli);
        }
        else{
    ...
    function detail($no_rekmedis){
      ...
      $sql_daftar = "SELECT pd.no_rekamedis, pd.no_rawat,ps.nama_pasien,pd.status_periksa,pd.status_pasien FROM tbl_pendaftaran as pd, tbl_pasien as ps WHERE pd.no_rekamedis = ps.no_rekamedis and pd.no_rawat = '$no_rawat'";
      ...
      $sql_data_tindakan = "SELECT * FROM tbl_riwayat_tindakan JOIN tbl_poli ON tbl_riwayat_tindakan.id_poli = tbl_poli.id_poli JOIN tbl_diagnosa_penyakit ON tbl_diagnosa_penyakit.kode_diagnosa = tbl_riwayat_tindakan.kode_penyakit JOIN tbl_tindakan ON tbl_tindakan.kode_tindakan = tbl_riwayat_tindakan.kode_tindakan JOIN icd ON icd.kode_icd = tbl_riwayat_tindakan.kode_icd WHERE tbl_riwayat_tindakan.no_rawat = '$no_rekmedis'";
    ...
    public function rawat_inap($no_rawat)
    {
        $this->db->where('no_rawat', $no_rawat);
        $object = array('status_periksa' => 'Rawat Inap', );
        $this->db->update('tbl_pendaftaran', $object);
        redirect('tindakanberobat/detail/'.$no_rawat,'refresh');
    }
  ...
  function cetakantrian($id){
   ...
   // }
    ...
    function autocompleteIcd() {
        $this->db->like('kode_icd', $_GET['term']);
        $this->db->select('kode_icd');
        $datatindakan = $this->db->get('icd')->result();
        foreach ($datatindakan as $tindakan) {
            $return_arr[] = $tindakan->kode_icd;
        }
   ...
   function rujukan_action(){
    ...
    public function edittindakan($id,$no_rawat) 
    {
        $row = $this->Tbl_pendaftaran_rekmed_model->getTindakanbyId($id);

        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('tindakanberobat/update_tindakan_action/'.$no_rawat),
                'id_riwayat_tindakan' => set_value('no_registrasi', $row->id_riwayat_tindakan),
                'no_rawat' => set_value('no_rawat', $row->no_rawat),
                'no_rekamedis' => set_value('no_rekamedis', $row->no_rekamedis),
                'id_poli' => set_value('id_poli', $row->id_poli),
                'kode_penyakit' => set_value('kode_penyakit', $row->kode_penyakit),
                'keluhan' => set_value('keluhan', $row->keluhan),
                'pemeriksaan_fisik' => set_value('pemeriksaan_fisik', $row->pemeriksaan_fisik),
                'pemeriksaan_lab' => set_value('pemeriksaan_lab', $row->pemeriksaan_lab),
                'kode_tindakan' => set_value('kode_tindakan', $row->kode_tindakan),
                'tanggal' => set_value('tanggal', $row->tanggal),
                'kode_icd' => set_value('kode_icd', $row->kode_icd),
        );
            $this->template->load('template','tindakanberobat/edittindakan', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tindakanberobat/detail/'.$no_rawat));
        }
    }
    
    public function update_tindakan_action($no_rawat) 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
                'id_riwayat_tindakan' => $this->input->post('id_riwayat_tindakan',TRUE),
                'no_rawat' => $this->input->post('no_rawat',TRUE),
                'no_rekamedis' => $this->input->post('no_rekamedis',TRUE),
                'id_poli' => $this->input->post('id_poli',TRUE),
                ...
                'pemeriksaan_lab' => $this->input->post('pemeriksaan_lab',TRUE),
                'kode_tindakan' => $this->input->post('kode_tindakan',TRUE),
                'tanggal' => date('d-m-Y'),
                'kode_icd' => $this->input->post('kode_icd',TRUE),
        );

            $this->Tbl_pendaftaran_rekmed_model->updatetindakan($this->input->post('id_riwayat_tindakan',TRUE), $data);
            $this->session->set_flashdata('message', '<div class="alert alert-info">Data Berhasil Diupdate
            </div>');  
            redirect(site_url('tindakanberobat/detail/'.$no_rawat));
        }
    ...
    public function delete($id,$level) 
    {
        ...
        if ($row) {
            ...
            </div>');                redirect(site_url('tindakanberobat/index/'.$level));
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tindakanberobat/index/'.$level));
        }
    }

    public function deletetindakan($id,$no_rawat) 
    {
        $row = $this->Tbl_pendaftaran_rekmed_model->getTindakanbyId($id);

        if ($row) {
            $this->Tbl_pendaftaran_rekmed_model->deletetindakan($id);
            $this->session->set_flashdata('message', '<div class="alert alert-danger">Data Berhasil Dihapus
            </div>');                redirect(site_url('tindakanberobat/detail/'.$no_rawat));
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('tindakanberobat/detail/'.$no_rawat));
        }
