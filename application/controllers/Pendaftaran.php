...
function cetakbulan(){
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(30, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(70, 7, '', 0, 0, 'C');
        $pdf->Cell(250, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        // mencetak string 
        $pdf->Cell(45, 7, '', 0, 0, 'C');
        $pdf->Cell(230, 7, 'PUSKESMAS MUNJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(30, 7, '', 0, 0, 'C');
        $pdf->Cell(270, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(70, 7, '', 0, 0, 'C');
        $pdf->Cell(190, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
  ...
  function cetakantrian($id){
        ...
        $pdf->Cell(90,5,'Kec. Munjungan, Kabupaten Trenggalek,66365',0,0,'C');
        $pdf->Cell(10,5,'',0,0,'C');
        $pdf->setFont('Arial','',7);
        $pdf->Cell(90,5,'',0,1,'L');
   ...
   function rujukan_action(){
    ...
    public function create() 
    {
        ...
        'dokter' => $this->Tbl_pendaftaran_model->getDokter()
    ...
    $sql_rekamedis    = "SELECT tr.*,pd.no_rawat,pd.tanggal_daftar 
        ...
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(235, 7, 'PUSKESMAS MUNJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(235, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(235, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
    ...
    public function cetaktanggal()
    {
		...
		if ($this->form_validation->run() == FALSE) {
            $this->template->load('template','pendaftaran/form_cetak');
		} else {
            $tanggal = $this->input->post('tanggal');
            $tanggal2 = $this->input->post('tanggal2');
            redirect('pendaftaran/cetaktanggal_action/'.$tanggal.'/'.$tanggal2,'refresh');
		}
    }

    public function cetaktanggal_action($tanggal,$tanggal2)
    {
        $no_rawat = substr($this->uri->uri_string(3), 27);
     
        $sql_data_resep = "SELECT pd.*, pol.nama_poli, dok.nama_dokter, pas.nama_pasien FROM tbl_pendaftaran as pd, tbl_poli as pol, tbl_dokter as dok, tbl_pasien as pas where pd.kode_dokter_penanggung_jawab=dok.kode_dokter AND pd.id_poli=pol.id_poli AND pd.no_rekamedis=pas.no_rekamedis AND pd.tanggal_daftar between '$tanggal' AND '$tanggal2' order by pd.no_rawat asc";
        ...
        $pdf->Image('assets/foto_profil/logo-rs.jpg', 100, 5, 30);
        //$pdf->Image('', )
        // mencetak string 
        $pdf->Cell(45, 7, '', 0, 0, 'C');
        $pdf->Cell(320, 7, 'PUSKESMAS MUNJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(40, 7, '', 0, 0, 'C');
        $pdf->Cell(335, 7, 'Krajanmunjungan, Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(80, 7, '', 0, 0, 'C');
        $pdf->Cell(247, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
        $pdf->Line(10,35, 440-20, 35);
        $pdf->Line(10,35.5, 440-20, 35.5);
        ...
        if($tanggal == $tanggal2){
            $pdf->Cell(250, 7, 'Laporan Data Pendaftaran Tanggal '.$tgl, 0, 1, 'C');
        }
        else{
            $tgl2 = date('d-m-Y', strtotime($tanggal2));
            $pdf->Cell(250, 7, 'Laporan Data Pendaftaran Tanggal '.$tgl.' Sampai Tanggal '.$tgl2, 0, 1, 'C');
        }
