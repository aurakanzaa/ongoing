...
class Kasir extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Tbl_kasir_model');
    ...
    public function index()
    {
        ...
        $pendaftaran                 = $this->Tbl_kasir_model->getSudahDiperiksa();
        ...
        $this->template->load('template', 'kasir/tbl_kasir_list', $data);
    }
    ...
    function detail($no_rekmed)
    {
        ...
        $sql_daftar    = "SELECT pd.no_rekamedis, pd.no_rawat,ps.nama_pasien,pd.status_periksa,pd.status_pasien FROM tbl_pendaftaran as pd, tbl_pasien as ps WHERE pd.no_rawat = '$no_rekmed' and pd.no_rekamedis = ps.no_rekamedis";
        $sql_kamar     = "SELECT pd.nama_pasien, pd.nama_kamar,pd.kelas_kamar,pd.harga,pd.tanggal_masuk,pd.tanggal_keluar FROM tbl_pengeluaran_kamar as pd WHERE pd.no_terima_kamar = '$no_rekmed'";
        
        // ini bisa
        // $sql_coba ="SELECT * FROM tbl_tindakan as tt RIGHT JOIN tbl_riwayat_tindakan as trt ON
        // tt.kode_tindakan = trt.kode_tindakan LEFT JOIN tbl_pengeluaran_kamar as pas ON
        // trt.no_rekamedis = pas.no_rekamedis";

        $sql_coba ="SELECT * FROM tbl_pendaftaran JOIN tbl_riwayat_tindakan ON tbl_riwayat_tindakan.no_rawat = tbl_pendaftaran.no_rawat JOIN tbl_tindakan ON tbl_tindakan.kode_tindakan = tbl_riwayat_tindakan.kode_tindakan WHERE tbl_pendaftaran.no_rawat = '$no_rekmed';";
        $data['data_tin'] = $this->db->query($sql_coba)->result();

        
        // $data['data_tin'] = $this->db->query($sql_new)->result();
        // $sql_tindakan  = "SELECT t.nama_tindakan, t.tindakan_oleh, t.harga FROM tbl_tindakan as t, tbl_riwayat_tindakan as riw WHERE t.kode_tindakan = '$kode_tindakan' and t.kode_tindakan = riw.kode_tindakan ";
        // $data['data_tindakan']  = $this->db->query($sql_tindakan)->result();
        ...
        // $sql_data_resep = "SELECT rt.nama_obat, rt.jenis_obat,rt.dosis_aturan_obat,rt.no_rawat,rt.tanggal, pkt.nama_pasien, tpo.harga_jual FROM tbl_resep_obat as rt, tbl_pasien as pkt, tbl_pengadaan_obat as tpo, tbl_obat as tbo WHERE rt.no_rawat = '$no_rekmed' and rt.no_rekamedis = pkt.no_rekamedis AND tbo.nama_obat = rt.nama_obat and tpo.kode_obat = tbo.kode_obat";
        $this->db->select('rt.nama_obat,rt.jumlah_obat, rt.jenis_obat,rt.dosis_aturan_obat,rt.no_rawat,rt.tanggal,tbo.harga_jual');
        $this->db->where('rt.no_rawat', $no_rekmed);
        // $this->db->join('tbl_pendaftaran', 'tbl_pendaftaran.no_rawat = rt.no_rawat');
        // $this->db->join('tbl_pasien as pas', 'tbl_pendaftaran.no_rekamedis = pas.no_rekamedis');
        $this->db->join('tbl_obat as tbo', 'tbo.nama_obat = rt.nama_obat');
        // $this->db->join('tbl_pengadaan_obat as tpo', 'tpo.kode_obat = tbo.kode_obat');
        $sql_data_resep = $this->db->get('tbl_resep_obat as rt')->result();
        ...
        $data['data_resep']  = $sql_data_resep;
        $data['data_kamar']  = $this->db->query($sql_kamar)->result();
        //$data['nama_obat'] = $this->db->query($sql_nama_obat)->row_array();
        
        $this->template->load('template', 'kasir/detail', $data);
    }
    ...
    public function bayar($no_rawat)
    {
        $this->db->where('no_rawat', $no_rawat);
        $object = array(
            'status_periksa' => 'Lunas'
        );
        $id_peng = substr($this->uri->uri_string(3), 23);
        $sql_resep = "SELECT * FROM tbl_resep_obat JOIN tbl_obat ON tbl_obat.nama_obat = tbl_resep_obat.nama_obat JOIN tbl_stok_obat ON tbl_stok_obat.kode_obat = tbl_obat.kode_obat JOIN tbl_pasien ON tbl_pasien.no_rekamedis = tbl_resep_obat.no_rekamedis WHERE tbl_resep_obat.no_rawat = '$no_rawat'";
        $resep = $this->db->query($sql_resep)->result();
        foreach ($resep as $key) {
            $jumlah = $key->jumlah_obat;
            $kode_obat = $key->kode_obat;
            $nama_pasien = $key->nama_pasien;
            $nama_obat = $key->nama_obat;
            $jenis_obat = $key->jenis_obat;
            $dosis_aturan_obat = $key->dosis_aturan_obat;
            $satuan = $key->satuan;
            // $id_peng = substr($this->uri->uri_string(3), 23);

            $sql_stok = "SELECT * FROM tbl_stok_obat WHERE tbl_stok_obat.kode_obat = '$kode_obat'";
            $stokobat = $this->db->query($sql_stok)->result();

            foreach ($stokobat as $val) {
                $stok = $val->jumlah;
            }
            $stokbaru = $stok - $jumlah;
            $update = "UPDATE tbl_stok_obat SET jumlah = '$stokbaru' WHERE kode_obat = '$kode_obat'";
            $this->db->query($update);

            $data = array('id_pengeluaran' => $id_peng,
                'no_terima_obat' => 'S-'.date('ymd').'-'.noPengeluaranOtomatis().'',
    ...
    function cetakku()
    {
        ...
        $sql_data_resep = "SELECT pd.*, pol.nama_poli, dok.nama_dokter, pas.nama_pasien FROM tbl_pendaftaran as pd, tbl_poli as pol, tbl_dokter as dok, tbl_pasien as pas where pd.kode_dokter_penanggung_jawab=dok.kode_dokter AND pd.id_poli=pol.id_poli AND pd.no_rekamedis=pas.no_rekamedis AND pd.status_periksa = 'Lunas' ";
        ...
        $pdf->Cell(5, 7, '', 0, 0, 'C');
        $pdf->Cell(30, 7, 'No Registrasi  ', 1, 0, 'C');
        $pdf->Cell(45, 7, 'No Rawat  ', 1, 0, 'C');
        ...
        $pdf->Cell(45, 7, 'Nama Poli  ', 1, 0, 'C');
        $pdf->Cell(40, 7, 'Status Pasien  ', 1, 1, 'C');
        ...
        foreach ($resep as $r) {
            ...
            $pdf->Cell(5, 7, '', 0, 0, 'C');
            $pdf->Cell(30, 7, $r->no_registrasi, 1, 0, 'C');
            $pdf->Cell(45, 7, $r->no_rawat, 1, 0, 'C');
            $pdf->Cell(40, 7, $r->no_rekamedis, 1, 0, 'C');
            $pdf->Cell(60, 7, $r->nama_pasien, 1, 0, 'C');
            $pdf->Cell(49, 7, $r->tanggal_daftar, 1, 0, 'C');
            $pdf->Cell(30, 7, $r->nama_dokter, 1, 0, 'C');
            $pdf->Cell(45, 7, $r->nama_poli, 1, 0, 'C');
            $pdf->Cell(40, 7, $r->status_pasien, 1, 1, 'C');
    ...
    function cetakantrian($id){
        ...
        $tgl = date('Y/m/d');
        $pdf->Image('assets/foto_profil/latar-kartu.jpg', 5, 5, 100, 56);
        $pdf->Image('assets/foto_profil/latar-kartu.jpg', 106, 5, 100, 56);
        ...
        $pdf->Cell(90, 5, 'Krajanmunjungan, Munjungan, Kabupaten Trenggalek, Jawa Timur 66365', 0, 0, 'C');
        $pdf->Cell(10, 5, '', 0, 0, 'C');
        $pdf->setFont('Arial', '', 7);
        $pdf->Cell(90, 5, '- Kartu ini hanya bisa digunakan untuk berobat di puskesmas munjungan.', 0, 1, 'L');
    ...
    function periksa_action(){
        ...
        $poli          = $this->db->get_where('tbl_poli', array(
            'nama_poli' => $nama_poli
        ))->row_array();
        $nama_penyakit = $this->input->post('nama_penyakit');
        $penyakit      = $this->db->get_where('tbl_diagnosa_penyakit', array(
            'nama_penyakit' => $nama_penyakit
        ))->row_array();
        $nama_dokter   = $this->input->post('nama_dokter');
        $dokter        = $this->db->get_where('tbl_dokter', array(
            'nama_dokter' => $nama_dokter
        ))->row_array();
        $nama_tindakan = $this->input->post('nama_tindakan');
        $tindakan      = $this->db->get_where('tbl_tindakan', array(
    ...
    function resep_action($no_rekmed){
        ...
        $obat              = $this->db->get_where('tbl_obat', array(
            'nama_obat' => $nama_obat
        ))->row_array();
        $jenis_obat        = $this->input->post('jenis_obat');
        $obatj             = $this->db->get_where('tbl_obat', array(
            'jenis_obat' => $jenis_obat
        ))->row_array();
        $dosis_aturan_obat = $this->input->post('dosis_aturan_obat');
        $jumlah_obat       = $this->input->post('jumlah_obat');
        $obatd             = $this->db->get_where('tbl_obat', array(
            'dosis_aturan_obat' => $dosis_aturan_obat
        ))->row_array();
        $no_rekamedis      = $this->input->post('no_rekamedis');
        ...
        redirect('kasir/detail/' . $no_rekmed);
    }
    ...
    function cetaknota($no_rekmed)
    {
        ...
        $sql_resep_obat = "SELECT * FROM tbl_pendaftaran JOIN tbl_resep_obat ON tbl_resep_obat.no_rawat = tbl_pendaftaran.no_rawat JOIN tbl_obat ON tbl_obat.nama_obat = tbl_resep_obat.nama_obat WHERE tbl_pendaftaran.no_rawat = '$no_rekmed'";
        // $this->db->join('tbl_pengadaan_obat as tpo', 'tpo.kode_obat = tbo.kode_obat');
        // $sql_data_resep = $this->db->query($sql_resep)->result();
        ...
        $pdf->Image('assets/foto_profil/logo-rs.jpg', 22, 5, 30);
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(268, 7, 'Puskesmas Munjungan', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(268, 7, 'Krajanmunjungan, Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(268, 7, 'Telepon : . E-mail: ', 0, 1, 'C');
        $pdf->Line(10, 35, 293 - 10, 35);
        $pdf->Line(10, 35.5, 293 - 10, 35.5);
        $pdf->Cell(7, 7, '', 0, 1);

        $pdf->Cell(268, 7, 'Nota Pembayaran', 0, 1, 'C');
        $pdf->Cell(7, 7, '', 0, 1);


        $pasien = $this->db->query($sql_daftar)->row_array();
        $pdf->Cell(30, 7, 'No.RM', 0, 0, 'L');
        $pdf->Cell(80, 7, ': ' . $pasien['no_rekamedis'], 0, 0, 'L'); //90 merupakan jarak antara no_rekamedis dengan isian ditangani oleh

        // $pasien = $this->db->query($sql_kamar)->row_array();
        

        $pdf->Cell(30, 7, 'No.Rawat', 0, 0, 'L');
        $pdf->Cell(80, 7, ': ' . $pasien['no_rawat'], 0, 0, 'L');

        $pdf->Cell(30, 7, 'Nama Pasien', 0, 0, 'L');
        $pdf->Cell(80, 7, ': ' . $pasien['nama_pasien'], 0, 0, 'L');
        ...
        // //data rekam medik
        // $pasien = $this->db->query($sql_no_rekmed)->row_array();
        // // var_dump($pasien);
        // $pdf->Cell(30, 7, '', 0, 0, 'L');
        // $pdf->Cell(30, 7, 'No.RM', 0, 0, 'L');
        ...
        //tabel hasil rekam medik
        $pdf->Cell(7, 7, '', 0, 1);
        $pdf->Cell(30, 7, 'Biaya Tindakan', 0, 1, 'C');
        $pdf->Cell(100, 7, 'Nama Tindakan', 1, 0, 'C');
        $pdf->Cell(100, 7, 'Harga', 1, 1, 'C');
        ...
        $tindakan = $this->db->query($sql_coba)->result();
        $ttltindakan = 0;
        foreach ($tindakan as $td) {
            $ttltindakan = $ttltindakan + $td->harga;
            // $pasien = $this->db->query($sql_daftar)->row_array();
            $pdf->Cell(100, 7, $td->nama_tindakan, 1, 0, 'C');
            $pdf->Cell(100, 7, $td->harga, 1, 1, 'C');
            // $pdf->Cell(45, 7, $td->nama_tindakan, 1, 0, 'C');
            // $pdf->Cell(69, 7, $td->hasil_periksa, 1, 0, 'C');
            // $pdf->Cell(39, 7, $td->nama_obat, 1, 0, 'C');
            // $pdf->Cell(45, 7, date('d-m-Y', strtotime($t->tanggal)), 1, 1, 'C');
        }
        $pdf->Cell(100, 7, 'SubTotal', 1,0 , 'C');
        if($pasien['status_pasien'] == 'BPJS'){
            $pdf->Cell(100, 7, $ttltindakan = '0', 1, 0, 'C');
        }
        else{
            $pdf->Cell(100, 7, $ttltindakan, 1, 0, 'C');
        }


        $pdf->Cell(7, 17, '', 0, 1);
        $pdf->Cell(35, 7, 'Biaya Resep Obat', 0, 1, 'C');
        $pdf->Cell(50, 7, 'Nama Obat', 1, 0, 'C');
        $pdf->Cell(50, 7, 'Jumlah Obat', 1, 0, 'C');
        ...
        $resep = $this->db->query($sql_resep_obat)->result();
        $hrgobat = 0;
        foreach ($resep as $rs) {
            // $pasien = $this->db->query($sql_daftar)->row_array();
            $pdf->Cell(50, 7, $rs->nama_obat, 1, 0, 'C');
            $pdf->Cell(50, 7, $rs->jumlah_obat, 1, 0, 'C');
            $pdf->Cell(50, 7, $rs->harga_jual, 1, 0, 'C');
            $pdf->Cell(50, 7, $hargatotal = $rs->jumlah_obat * $rs->harga_jual, 1, 1, 'C');
            $hrgobat = $hargatotal + $hrgobat;
            // $pdf->Cell(45, 7, $td->nama_tindakan, 1, 0, 'C');
            // $pdf->Cell(69, 7, $td->hasil_periksa, 1, 0, 'C');
            // $pdf->Cell(39, 7, $td->nama_obat, 1, 0, 'C');
            // $pdf->Cell(45, 7, date('d-m-Y', strtotime($t->tanggal)), 1, 1, 'C');
        ...
        $pdf->Cell(150, 7, 'SubTotal', 1,0 , 'C');
            $pdf->Cell(50, 7, $hrgobat = '0', 1, 0, 'C');

        $pdf->Cell(7, 17, '', 0, 1);
        $pdf->Cell(35, 7, 'Biaya Rawat Inap', 0, 1, 'C');
        $pdf->Cell(35, 7, 'Nama Kamar', 1, 0, 'C');
        $pdf->Cell(35, 7, 'Kelas Kamar', 1, 0, 'C');
        $pdf->Cell(35, 7, 'Tanggal Masuk', 1, 0, 'C');
        $pdf->Cell(35, 7, 'Tanggal Keluar', 1, 0, 'C');
        $pdf->Cell(35, 7, 'Harga/Hari', 1, 0, 'C');
        ...
        $kamar = $this->db->query($sql_kamar)->result();
        $ttlkamar = 0;
        foreach ($kamar as $km) {
        $date1=date_create($km->tanggal_masuk);
        $date2=date_create($km->tanggal_keluar);
        $diff=date_diff($date1,$date2);
        $hari = $diff->format("%a");
        if ($pasien['status_pasien'] == "BPJS") {
            $ttlkamar = '0';
        }else{
            $ttlkamar = $km->harga * $hari;
        }
            // $pasien = $this->db->query($sql_daftar)->row_array();
            $pdf->Cell(35, 7, $km->nama_kamar, 1, 0, 'C');
            $pdf->Cell(35, 7, $km->kelas_kamar, 1, 0, 'C');
            $pdf->Cell(35, 7, $km->tanggal_masuk, 1, 0, 'C');
            $pdf->Cell(35, 7, $km->tanggal_keluar, 1, 0, 'C');
            $pdf->Cell(35, 7, $km->harga, 1, 0, 'C');
            $pdf->Cell(35, 7, $ttlkamar, 1, 1, 'C');
        ...
        $pdf->Cell(175, 7, 'SubTotal', 1,0 , 'C');
        // if($pasien['status_pasien'] == 'BPJS'){
            // $pdf->Cell(35, 7, $ttlkamar = '0', 1, 0, 'C');
        // }
        // else{
            $pdf->Cell(35, 7, $ttlkamar, 1, 0, 'C');
        // }


        $pdf->Cell(7, 10, '', 0, 1);
        $pdf->Cell(35, 7, 'Biaya Pendaftaran', 0, 1, 'C');
        $pdf->Cell(50, 7, 'Biaya Pendaftaran', 1, 0, 'C');
        
        
        if ($pasien['status_pasien'] == "BPJS") {
            $pdf->Cell(35, 7, $ttlpendaftaran ='0', 1, 0, 'C');
        }else{
            $pdf->Cell(35, 7, $ttlpendaftaran ='10000', 1, 0, 'C');
        }

        $pdf->Cell(7, 17, '', 0, 1);
        $pdf->Cell(23, 7, 'Grand Total', 0, 1, 'C');
        $pdf->Cell(50, 7, 'Grand Total', 1, 0, 'C');
        
            $pdf->Cell(35, 7, $ttlpendaftaran + $ttlkamar + $hrgobat + $ttltindakan, 1, 0, 'C');
    ...
    function cetakrekamedis(){
        ...
        $pdf->Cell(268, 7, 'Puskesmas Munjungan', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(268, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(268, 7, 'Telepon : . E-mail: ', 0, 1, 'C');
        $pdf->Line(10, 35, 293 - 10, 35);
        $pdf->Line(10, 35.5, 293 - 10, 35.5);
        $pdf->Cell(7, 7, '', 0, 1);
    ...
    function cetakresepobat($no_rekmed)
    {
        
        // $no_rawat = substr($this->uri->uri_string(3), 31);
        $sql_daftar     = "SELECT pd.no_rekamedis, pd.no_rawat,pd.tanggal_daftar,ps.nama_pasien, td.nama_dokter FROM tbl_pendaftaran as pd, tbl_pasien as ps, tbl_dokter as td WHERE pd.no_rekamedis = ps.no_rekamedis and td.kode_dokter = pd.kode_dokter_penanggung_jawab and pd.no_rawat = '$no_rekmed'";
        //   $sql_penyakit = "SELECT pd.no_rekamedis, pd.no_rawat,dg.nama_penyakit FROM tbl_pendaftaran as pd, tbl_diagnosa_penyakit as dg WHERE pd.no_rekamedis and pd.no_rawat = '$no_rawat'";
        //   $sql_poli_input_rekmed = "SELECT pd.no_rawat,pd.id_poli, p.nama_poli FROM tbl_pendaftaran as pd, tbl_poli as p WHERE pd.no_rawat = '$no_rawat' AND pd.id_poli = p.id_poli";
        $sql_data_resep = "SELECT rt.nama_obat, rt.jenis_obat,rt.dosis_aturan_obat,rt.jumlah_obat,rt.no_rawat,rt.tanggal, pkt.nama_pasien FROM tbl_resep_obat as rt, tbl_pasien as pkt WHERE rt.no_rawat = '$no_rekmed' AND rt.no_rekamedis = pkt.no_rekamedis";
        ...
        $pdf->Image('assets/foto_profil/logo-rs.jpg', 4, 5, 30);
        //$pdf->Image('', )
        // mencetak string 
        $pdf->Cell(190, 7, 'PUSKESMAS MUNJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
        $pdf->Cell(190, 7, 'Munjungan Krajan, Kec. Munjungan, Kabupaten Trenggalek, Jawa Timur 66365.', 0, 1, 'C');
        $pdf->Cell(190, 7, 'Telepon : (0267) 4218872. E-mail: ', 0, 1, 'C');
    ...
    function cetakrujukan()
    {
        ...
        $pdf->Image('http://puskesmasmunjungan.com/assets/foto_profil/logo-rs.jpg', 4, 5, 30);
        //$pdf->Image('', a)
        // mencetak string 
        $pdf->Cell(190, 7, 'PUSKESMAS MUNJUNGAN', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 12);
