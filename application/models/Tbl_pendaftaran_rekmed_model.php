...
// halo
    ...
    function get_limit_data_poli($limit, $start = 0, $poli) {
        $this->db->where('tbl_poli.nama_poli', $poli);
        $this->db->where('tbl_pendaftaran.tanggal_daftar', date('Y-m-d'));
        $this->db->order_by('tbl_pendaftaran.no_rawat', 'ASC');
        $this->db->join('tbl_pasien', 'tbl_pendaftaran.no_rekamedis = tbl_pasien.no_rekamedis');
        $this->db->join('tbl_dokter', 'tbl_pendaftaran.kode_dokter_penanggung_jawab = tbl_dokter.kode_dokter');
    ...
    function get_limit_data_poli2($limit, $start = 0, $poli) {
        $this->db->where('tbl_poli.nama_poli', $poli);
        // $this->db->where('tbl_pendaftaran.tanggal_daftar', date('Y-m-d'));
        $this->db->order_by('tbl_pendaftaran.no_rawat', 'ASC');
        $this->db->join('tbl_pasien', 'tbl_pendaftaran.no_rekamedis = tbl_pasien.no_rekamedis');
    ...
    function updatetindakan($id, $data)
    {
        $this->db->where('id_riwayat_tindakan', $id);
        $this->db->update('tbl_riwayat_tindakan',$data);
    }
    ...
    {
        $this->db->where('id_riwayat_tindakan', $id);
        $this->db->delete('tbl_riwayat_tindakan');
    }
